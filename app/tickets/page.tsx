...
import { deleteTicket } from "@/lib/actionTickets";
    ...
    <main className="flex min-h-screen flex-col items-center gap-10 p-24 max-w-5xl mx-auto">
      ...
      {result.length > 0 ? (
        <ul className="w-full flex gap-5 flex-col">
          {result.map((ticket) => (
            <li key={ticket.id} className="p-5 rounded-md bg-zinc-100 shadow-md flex gap-5 justify-between w-full">
              <Link href={`/tickets/update/${ticket.id}`} className="flex-1">
                <p>
                  {ticket.name}
                  <br />
      ...
      <div className="flex gap-5 flex-wrap">
        <Link href="/tickets/create" className="text-blue-700">
          Create a new ticket
        </Link>
        <Link href="/tickets/create/client" className="text-blue-700">
